...
st.plotly_chart(fig_burbujas, use_container_width=True, key="mapa")
...
if "MANERA_MUERTE" in df.columns and "Detalle" in df.columns and "MUNICIPIO" in df.columns:
    ...
    st.warning("⚠️ Faltan columnas para ciudades violentas.")
...
if "DEPARTAMENTO" in df.columns and "SEXO" in df.columns:
    sexo_map = {1: "Hombre", 2: "Mujer", 3: "Sin especificar"}
    df["SEXO_LABEL"] = df["SEXO"].map(sexo_map)
    sexo_dep = df.groupby(["DEPARTAMENTO", "SEXO_LABEL"]).size().reset_index(name="Total")
    fig_apiladas = px.bar(
        sexo_dep, x="DEPARTAMENTO", y="Total", color="SEXO_LABEL",
        title="Muertes por sexo en cada departamento",
        labels={"SEXO_LABEL": "Sexo"}
    )
    fig_apiladas.update_layout(xaxis_tickangle=-45)
    st.plotly_chart(fig_apiladas, use_container_width=True, key="sexo_dep")
else:
    st.warning("⚠️ Faltan columnas para muertes por sexo y departamento.")
...
if "HORA" in df.columns and "MINUTOS" in df.columns:
    ...
    st.warning("⚠️ Faltan columnas para graficar hora y minutos.")